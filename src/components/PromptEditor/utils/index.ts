...
import { NodeVariables } from '../types'
...
export function getSelectedNode(
  ...
  if (isBackward)
    return $isAtNodeEnd(focus as any) ? anchorNode : focusNode
  else
    return $isAtNodeEnd(anchor as any) ? anchorNode : focusNode
}
...
export const getWorkflowNodesMap = (variables: NodeVariables[]) => {
  return variables.reduce((map: any, item:NodeVariables) => {
    if(item.id) {
      map[item.id] = {
        title: item.name
      }
